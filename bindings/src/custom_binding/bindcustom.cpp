...
CustomDataStruct *
copy_custom_struct (void *data, void *user_data)
{
  NvDsUserMeta *srcMeta = (NvDsUserMeta *)data;
  CustomDataStruct *srcData = (CustomDataStruct *)srcMeta->user_meta_data;
  CustomDataStruct *destData
      = (CustomDataStruct *)g_malloc0 (sizeof (CustomDataStruct));
...
void
release_custom_struct (void *data, void *user_data)
{
  NvDsUserMeta *srcMeta = (NvDsUserMeta *)data;
  if (srcMeta != nullptr)
    {
...
bindcustom (py::module &m)
{
  ...
  py::class_<CustomDataStruct> (m, "CustomDataStruct",
      ...
      .def_property ("message",
                     STRING_FREE_EXISTING (CustomDataStruct, message))
      .def_readwrite ("sampleInt", &CustomDataStruct::sampleInt)

      .def (
          "cast", [] (void *data) { return (CustomDataStruct *)data; },
  ...
  m.def (
      "alloc_custom_struct",
      [] (NvDsUserMeta *meta) {
        auto *mem = (CustomDataStruct *)g_malloc0 (sizeof (CustomDataStruct));
        meta->base_meta.copy_func
            = (NvDsMetaCopyFunc)pydeepstream::copy_custom_struct;
        meta->base_meta.release_func
            = (NvDsMetaReleaseFunc)pydeepstream::release_custom_struct;
        return mem;
      },
      py::return_value_policy::reference,
